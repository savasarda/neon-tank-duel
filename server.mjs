...
function snapshot(room,includeWalls=false){const now=Date.now();return{code:room.code,maxPlayers:room.maxPlayers,mapLayout:room.mapLayout,phase:room.phase,countdown:room.countdown,roundId:room.roundId,theme:room.theme,explosion:room.explosion,players:room.players.map(player=>publicPlayer(player,now)),bullets:room.bullets,trails:room.trails,...(includeWalls?{walls:room.walls}:{}),mines:room.mines,powerups:room.powerups,reactions:room.reactions,winner:room.winner}}
...
function tick(room){
  ...
  room.mines=room.mines.filter(mine=>mine.life>0);room.networkTick=(room.networkTick||0)+1;if(room.networkTick%3===0)broadcast(room);
}
