...
const W = 1400, H = 1000, CELL = 100, TANK_R = 40, WIN_SCORE = 5;
...
function maze() { // rotationally symmetric walls, clear spawn zones
  ...
  for(let y=1;y<9;y++) for(let x=2;x<7;x++) if(!((x<3&&y<3)||(x>5&&y>6))) candidates.push([x,y]);
  for(let i=0;i<12;i++){ const [x,y]=candidates[Math.floor(Math.random()*candidates.length)]; const key=`${x},${y}`; if(walls.some(w=>w.key===key)) {i--;continue}; walls.push({key,x:x*CELL,y:y*CELL,w:CELL,h:CELL},{key:`${13-x},${9-y}`,x:(13-x)*CELL,y:(9-y)*CELL,w:CELL,h:CELL}); }
  return walls.map(({key,...w})=>w);
}
function newRound(room){ room.walls=maze(); room.bullets=[]; room.phase='playing'; room.players[0].x=150;room.players[0].y=150;room.players[0].a=Math.PI/4;room.players[1].x=1250;room.players[1].y=850;room.players[1].a=-3*Math.PI/4; room.players.forEach(p=>p.cooldown=0); }
...
function tick(room){ if(room.phase!=='playing')return; for(const p of room.players){p.cooldown=Math.max(0,p.cooldown-1);const i=p.input||{}; if(Number.isFinite(i.heading))p.a=i.heading; const speed=clamps(i.move||0,0,1)*4.6; const nx=p.x+Math.cos(p.a)*speed,ny=p.y+Math.sin(p.a)*speed;if(!blocked(room,nx,ny)){p.x=nx;p.y=ny;} }
...
io.on('connection', socket=>{
 ...
 socket.on('join-room', raw=>{const c=String(raw||'').toUpperCase(),room=rooms.get(c);if(!room)return socket.emit('room-error','Oda bulunamadı.');if(room.players.length>=2)return socket.emit('room-error','Bu oda dolu.');room.players.push({id:socket.id,x:1250,y:850,a:-2.36,score:0,input:{},cooldown:0});socket.join(c);newRound(room);socket.emit('room-joined',{code:c,slot:1,state:snapshot(room)});broadcast(room);});
 ...
 socket.on('fire',({code:c,angle})=>{const r=rooms.get(c),idx=r?.players.findIndex(p=>p.id===socket.id),p=r?.players[idx];if(!p||r.phase!=='playing'||p.cooldown>0||r.bullets.some(b=>b.owner===idx))return;const a=Number.isFinite(angle)?angle:p.a;p.a=a;p.cooldown=22;r.bullets.push({x:p.x+Math.cos(a)*50,y:p.y+Math.sin(a)*50,vx:Math.cos(a)*11,vy:Math.sin(a)*11,life:115,bounces:0,owner:idx});});
