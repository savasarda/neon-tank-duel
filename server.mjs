...
io.on('connection',socket=>{
  ...
  socket.on('create-room',({speed,players,layout,profile}={})=>{const code=roomCode(),maxPlayers=clamp(Math.round(Number(players)||2),2,4),mapLayout=MAP_LAYOUTS[layout]?layout:'classic',tankSpeed=SPEEDS[speed]||SPEEDS.normal,arena=arenaFor(mapLayout),room={code,maxPlayers,mapLayout,tankSpeed,theme:'neon',roundId:0,explosion:null,spawns:arena.spawns,players:[{...assignProfile(profile),id:socket.id,ready:false,x:arena.spawns[0].x,y:arena.spawns[0].y,a:.78,turret:.78,score:0,input:{},cooldown:0,effects:freshEffects()}],walls:arena.walls,bullets:[],trails:[],mines:[],powerups:[],reactions:[],phase:'waiting'};rooms.set(code,room);socket.join(code);socket.emit('room-joined',{code,slot:0,state:snapshot(room,true)})});
  socket.on('create-bot-game',({speed,profile}={})=>{const code=roomCode(),tankSpeed=SPEEDS[speed]||SPEEDS.normal,arena=arenaFor('classic'),room={code,maxPlayers:2,mapLayout:'classic',tankSpeed,theme:'neon',roundId:0,explosion:null,spawns:arena.spawns,players:[{...assignProfile(profile),id:socket.id,ready:true,score:0,effects:freshEffects()},{...assignProfile({name:'Bot',badge:'◆'},[assignProfile(profile)]),id:`bot-${code}`,isBot:true,ready:true,score:0,effects:freshEffects()}],walls:arena.walls,bullets:[],trails:[],mines:[],powerups:[],reactions:[],phase:'waiting'};newRound(room);rooms.set(code,room);socket.join(code);socket.emit('room-joined',{code,slot:0,state:snapshot(room,true)});broadcast(room)});
