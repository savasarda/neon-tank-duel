...
function tick(room){ if(room.phase!=='playing')return; for(const p of room.players){p.cooldown=Math.max(0,p.cooldown-1);const i=p.input||{}; if(Number.isFinite(i.heading))p.a=i.heading;if(Number.isFinite(i.aim))p.turret=i.aim; const speed=clamps(i.move||0,0,1)*room.tankSpeed; const nx=p.x+Math.cos(p.a)*speed,ny=p.y+Math.sin(p.a)*speed;if(!blocked(room,nx,ny)){p.x=nx;p.y=ny;} }
...
io.on('connection', socket=>{
 ...
 socket.on('fire',({code:c,angle})=>{const r=rooms.get(c),idx=r?.players.findIndex(p=>p.id===socket.id),p=r?.players[idx];if(!p||r.phase!=='playing'||p.cooldown>0||r.bullets.filter(b=>b.owner===idx).length>=5)return;const a=Number.isFinite(angle)?angle:p.turret;p.turret=a;p.cooldown=14;r.bullets.push({x:p.x+Math.cos(a)*54,y:p.y+Math.sin(a)*54,vx:Math.cos(a)*11,vy:Math.sin(a)*11,life:115,bounces:0,owner:idx});});
