...
function tick(room){ if(room.phase!=='playing')return; for(const p of room.players){p.cooldown=Math.max(0,p.cooldown-1);const i=p.input||{}; if(Number.isFinite(i.heading))p.a=i.heading;p.turret=p.a; const speed=clamps(i.move||0,0,1)*room.tankSpeed/2;const moved=moveCircle(room.walls,p.x,p.y,Math.cos(p.a)*speed,Math.sin(p.a)*speed,TANK_R+8,W,H);p.x=moved.x;p.y=moved.y;}
