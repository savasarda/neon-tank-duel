...
import { SURVIVOR_ARENA, SPAWN_SHIELD_MS, pickRespawn, applySurvivorDeath } from './survivor.mjs';
...
const CLASSIC_ARENA={width:W,height:H,radius:COLLISION_R,tankRadius:TANK_R};
const REACTIONS=['😄','😎','😱','🔥','💥','👋'];
...
function arenaFor(layout,arena=CLASSIC_ARENA){return createMaze(arena.width,arena.height,WALL,MAP_LAYOUTS[layout]||MAP_LAYOUTS.classic)}
...
function placePlayer(player,spawn,angle){Object.assign(player,{x:spawn.x,y:spawn.y,a:angle,turret:angle,cooldown:0,input:{},inputQueue:createInputQueue(),motionProtocol:false,selfHits:0,respawnAt:0,invulnerableUntil:0,spawnSerial:(player.spawnSerial||0)+1,effects:freshEffects()})}
...
function newRound(room){
  const arena=arenaFor(room.mapLayout,room.arena);room.walls=arena.walls;room.spawns=arena.spawns;room.bullets=[];room.trails=[];room.mines=[];room.powerups=[];room.reactions=[];
  ...
  room.spawnOrder=nextSpawnOrder(room,arena.spawns.length);room.players.forEach((player,index)=>{const spawn=arena.spawns[room.spawnOrder[index]],angle=Math.atan2(room.arena.height/2-spawn.y,room.arena.width/2-spawn.x);placePlayer(player,spawn,angle)});
}
function publicPlayer(player,now){const{id,input,cooldown,inputQueue,motionProtocol,wasPhasing,...visible}=player;return{...visible,lastProcessedInput:inputQueue?.processed??0,moving:!player.respawnAt&&(player.input?.move||0)>.05,effects:{speed:player.effects.speedUntil>now,double:player.effects.doubleUntil>now,shield:player.effects.shieldUntil>now,invisible:player.effects.invisibleUntil>now,range:player.effects.rangeUntil>now,bulletSpeed:player.effects.bulletSpeedUntil>now,phase:player.effects.phaseUntil>now,mines:player.effects.mines,rockets:player.effects.rockets,pierces:player.effects.pierces}}}
function snapshot(room,includeWalls=false){const now=Date.now();return{motionVersion:1,serverTime:now,code:room.code,maxPlayers:room.maxPlayers,mapLayout:room.mapLayout,gameMode:room.gameMode,arena:room.arena,tankSpeed:room.tankSpeed,selfDamage:room.selfDamage,phase:room.phase,countdown:room.countdown,roundId:room.roundId,theme:room.theme,explosion:room.explosion,players:room.players.map(player=>publicPlayer(player,now)),bullets:room.bullets,trails:room.trails,...(includeWalls?{walls:room.walls}:{}),mines:room.mines,powerups:room.powerups,reactions:room.reactions,winner:room.winner}}
...
function respawnPlayers(room,now){
  for(const player of room.players){
    if(!player.respawnAt||now<player.respawnAt)continue;
    const spawn=pickRespawn(room);if(!spawn)continue;
    placePlayer(player,spawn,Math.random()*Math.PI*2);player.invulnerableUntil=now+SPAWN_SHIELD_MS;
...
function spawnPowerup(room){
  for(let attempt=0;attempt<100;attempt++){
    const {width,height}=room.arena,x=80+Math.random()*(width-160),y=80+Math.random()*(height-160);
    if(circleTouchesWorld(room.walls,x,y,28,width,height))continue;
    if(room.players.some(player=>!player.respawnAt&&(player.x-x)**2+(player.y-y)**2<150**2))continue;
...
function exitPhaseSafely(room,target){
  const player=room.players[target],{width,height,radius}=room.arena;if(!circleTouchesWorld(room.walls,player.x,player.y,radius,width,height))return;
  for(let distance=6;distance<=180;distance+=6)for(let step=0;step<24;step++){
    const angle=step*Math.PI/12,x=player.x+Math.cos(angle)*distance,y=player.y+Math.sin(angle)*distance;
    if(!circleTouchesWorld(room.walls,x,y,radius,width,height)&&!room.players.some((other,index)=>index!==target&&!other.respawnAt&&(other.x-x)**2+(other.y-y)**2<(radius*2)**2)){player.x=x;player.y=y;return}
  }
  Object.assign(player,room.spawns[room.spawnOrder?.[target]??target]);
}
function addBullet(room,owner,angle,type='normal'){
  const player=room.players[owner],rocket=type==='rocket',piercing=type==='pierce',longRange=player.effects.rangeUntil>Date.now(),boosted=player.effects.bulletSpeedUntil>Date.now(),offset=room.gameMode==='survivor'?14:20,speed=(rocket?8:piercing?7:5.5)*(boosted?1.65:1),size=room.gameMode==='survivor'?.7:1;
  room.nextBulletId=(room.nextBulletId||0)+1;
  room.bullets.push({id:room.nextBulletId,x:player.x+Math.cos(angle)*offset,y:player.y+Math.sin(angle)*offset,vx:Math.cos(angle)*speed,vy:Math.sin(angle)*speed,radius:(rocket?22:piercing?14:BULLET_R)*size,life:rocket?220:piercing?320:longRange?480:300,bounces:0,maxBounces:rocket?3:6,owner,type});
}
function fireForPlayer(room,owner,now=Date.now()){
  const player=room.players[owner];if(!player||player.respawnAt||player.cooldown>0)return false;
...
function updateBot(room,index,now){
  const bot=room.players[index],target=room.players[index===0?1:0];if(!bot||!target||bot.respawnAt||target.respawnAt)return;
...
function absorbOrEnd(room,target,winner){const player=room.players[target];if(player.respawnAt||player.invulnerableUntil>Date.now())return false;if(player.effects.shieldUntil>Date.now()){player.effects.shieldUntil=0;return false}defeat(room,winner,target);return true}
...
function tick(room){
  ...
  if(room.gameMode==='survivor')respawnPlayers(room,now);
  ...
  room.players.forEach((player,index)=>{if(player.respawnAt)return;const phasing=player.effects.phaseUntil>now;if(!phasing&&player.wasPhasing)exitPhaseSafely(room,index);player.wasPhasing=phasing});

  const current=room.players.map(player=>({x:player.x,y:player.y}));
  const proposed=room.players.map((player,index)=>{
    if(player.respawnAt)return current[index];
    player.cooldown=Math.max(0,player.cooldown-1);
    const speed=room.tankSpeed*(player.effects.speedUntil>now?1.7:1);
    const others=current.filter((_,other)=>other!==index&&!room.players[other].respawnAt);
    let commands;
    ...
    for(const command of commands){position=advanceTank(position,{move:command.move||0,heading:Number.isFinite(command.heading)?command.heading:position.a},player.effects.phaseUntil>now?[]:room.walls,speed,others,room.arena);player.input=command}
    player.a=position.a;player.turret=position.a;return position;
  });
  const liveIndexes=room.players.map((player,index)=>player.respawnAt?-1:index).filter(index=>index>=0);
  const positions=preventMultipleCircleOverlap(liveIndexes.map(index=>current[index]),liveIndexes.map(index=>proposed[index]),room.arena.radius);
  liveIndexes.forEach((index,position)=>Object.assign(room.players[index],positions[position]));

  for(let playerIndex=0;playerIndex<room.players.length;playerIndex++){
    const player=room.players[playerIndex];
    if(player.respawnAt)continue;
    room.powerups=room.powerups.filter(power=>{if((player.x-power.x)**2+(player.y-power.y)**2<(room.arena.radius+24)**2){grantPower(player,power.type,now);return false}return true});
  }

  room.trails=room.trails.filter(trail=>--trail.life>0);
  room.reactions=room.reactions.filter(reaction=>--reaction.life>0);
  for(const bullet of room.bullets){
    const oldVx=bullet.vx,oldVy=bullet.vy,radius=bullet.radius||BULLET_R,bounced=bullet.type==='pierce'?advancePiercingBullet(bullet,radius,room.arena.width,room.arena.height):advanceBullet(bullet,room.walls,radius,room.arena.width,room.arena.height);bullet.life--;bullet.bounces+=bounced?1:0;
    ...
    const hit=room.players.findIndex((player,index)=>!player.respawnAt&&player.invulnerableUntil<=now&&(index!==bullet.owner||(room.selfDamage&&bullet.bounces>0))&&(player.x-bullet.x)**2+(player.y-bullet.y)**2<(room.arena.tankRadius+radius)**2);
    if(hit>=0){
      bullet.life=0;
      if(hit===bullet.owner){
        ...
        if(room.players[hit].selfHits>=3){defeat(room,selfDamageWinner(room,hit),hit);return}
  ...
  for(const mine of room.mines){mine.life--;mine.armed--;if(mine.armed>0)continue;const hit=room.players.findIndex((player,index)=>!player.respawnAt&&player.invulnerableUntil<=now&&index!==mine.owner&&(player.x-mine.x)**2+(player.y-mine.y)**2<(room.arena.radius+25)**2);if(hit>=0){mine.life=0;if(absorbOrEnd(room,hit,mine.owner))return}}
...
io.on('connection',socket=>{
  ...
  socket.on('create-room',({speed,players,layout,selfDamage,gameMode,profile}={})=>{const code=roomCode(),maxPlayers=clamp(Math.round(Number(players)||2),2,4),mapLayout=MAP_LAYOUTS[layout]?layout:'classic',mode=gameMode==='survivor'?'survivor':'classic',dimensions=mode==='survivor'?SURVIVOR_ARENA:CLASSIC_ARENA,tankSpeed=SPEEDS[speed]||SPEEDS.normal,arena=arenaFor(mapLayout,dimensions),room={code,maxPlayers,mapLayout,gameMode:mode,arena:dimensions,tankSpeed,selfDamage:selfDamage===true,theme:'neon',roundId:0,explosion:null,spawns:arena.spawns,players:[{...assignProfile(profile),id:socket.id,ready:false,x:arena.spawns[0].x,y:arena.spawns[0].y,a:.78,turret:.78,score:0,input:{},cooldown:0,selfHits:0,effects:freshEffects()}],walls:arena.walls,bullets:[],trails:[],mines:[],powerups:[],reactions:[],phase:'waiting'};rooms.set(code,room);socket.join(code);socket.emit('room-joined',{code,slot:0,state:snapshot(room,true)})});
  socket.on('create-bot-game',({speed,gameMode,profile}={})=>{const code=roomCode(),mode=gameMode==='survivor'?'survivor':'classic',dimensions=mode==='survivor'?SURVIVOR_ARENA:CLASSIC_ARENA,tankSpeed=SPEEDS[speed]||SPEEDS.normal,arena=arenaFor('classic',dimensions),room={code,maxPlayers:2,mapLayout:'classic',gameMode:mode,arena:dimensions,tankSpeed,selfDamage:false,theme:'neon',roundId:0,explosion:null,spawns:arena.spawns,players:[{...assignProfile(profile),id:socket.id,ready:true,score:0,effects:freshEffects()},{...assignProfile({name:'Bot',badge:'◆'},[assignProfile(profile)]),id:`bot-${code}`,isBot:true,ready:true,score:0,effects:freshEffects()}],walls:arena.walls,bullets:[],trails:[],mines:[],powerups:[],reactions:[],phase:'waiting'};newRound(room);rooms.set(code,room);socket.join(code);socket.emit('room-joined',{code,slot:0,state:snapshot(room,true)});broadcast(room)});
  ...
  socket.on('input-batch',({code,roundId,spawnSerial,commands}={})=>{
    const room=rooms.get(code),player=room?.players.find(item=>item.id===socket.id);
    if(!player||player.respawnAt||room.phase!=='playing'||roundId!==room.roundId||(spawnSerial??player.spawnSerial)!==player.spawnSerial)return;
